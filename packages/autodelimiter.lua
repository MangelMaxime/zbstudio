...
return {
  ...
  onEditorCharAdded = function(self, editor, event)
    ...
    elseif pairs[char] then
      local curcode = editor:GetCharAt(curpos)
      -- if the entered matches opening delimiter and is not directly before a non empty text, then insert the pair
      if curcode == 13 or curcode == 32 then
